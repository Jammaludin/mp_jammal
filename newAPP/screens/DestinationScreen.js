...
export default function DestinationScreen(props) {
    ...
    return (
        <View className="bg-white flex-1">
            ...
            {/* back button */}
            <SafeAreaView className={'flex-row -between items-center w-full absolute' + topMargin}>
                <TouchableOpacity onPress={() => navigation.goBack()} className="p-2 rounded-full ml-4" style={{ backgroundColor: 'rgba(255,255,255,0.5)' }}>
                    <ChevronLeftIcon size={wp(7)} strokeWidth={4} color="white" />
                </TouchableOpacity>
                <TouchableOpacity onPress={() => toggleFavourite(!isFavourite)} className="p-2 rounded-full mr-4" style={{ backgroundColor: 'rgba(255,255,255,0.5)' }}>
